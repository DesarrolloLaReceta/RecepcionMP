 ...
 * En PROD / staging:                 MsalAuthProvider usa Entra ID.
 *
...
// Exportado para que MockAuthProvider pueda proveer al mismo contexto
...
export function useAuth(): AuthContextValue {
  ...
  if (!ctx) throw new Error("useAuth debe usarse dentro de <AuthProvider> o <MockAuthProvider>");
  return ctx;
}
...
function MsalAuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const getAccessToken = useCallback(async (): Promise<string> => {
    ...
    try {
      const res = await instance.acquireTokenSilent({ ...silentRequest, account: active });
      return res.accessToken;
    } catch (err) {
      if (err instanceof InteractionRequiredAuthError) {
        const res = await instance.acquireTokenPopup({ ...loginRequest, account: active });
        return res.accessToken;
      }
      throw err;
    }
  ...
  return (
    <AuthContext.Provider value={{
      ...
      displayName, email, roles, initials,
      hasRole, getAccessToken, login, logout,
    }}>
      {children}
...
// ─── PROVIDER EXPORTADO ───────────────────────────────────────────────────────
// Solo se usa en modo PRODUCCIÓN (main.tsx lo monta dentro de MsalProvider).
