...
function LotesBar({ total, liberados, rechazados }: { total: number; liberados: number; rechazados: number }) {
  if (total === 0) return <span style={{ fontSize: "var(--text-xs)", color: "var(--text-tertiary)" }}>—</span>;
  return (
    <div className="rp-lotes-bar">
      <div className="rp-lotes-track">
        {liberados  > 0 && <div className="rp-lotes-liberados"  style={{ width: `${(liberados  / total) * 100}%` }} />}
        {rechazados > 0 && <div className="rp-lotes-rechazados" style={{ width: `${(rechazados / total) * 100}%` }} />}
      </div>
      <span className="rp-lotes-count">{liberados}/{total}</span>
    </div>
  );
}
  ...
  return (
    <tr className="rp-row" onClick={onClick}>
      ...
      <td className="rp-td-last">
        <svg className="rp-chevron" width="14" height="14" viewBox="0 0 24 24" fill="none" strokeWidth="2" strokeLinecap="round">
    ...
    <div className="rp-page">
      ...
      <div className="rp-header">
        ...
        <div className="rp-header-actions">
          <Button variant="ghost" size="sm" loading={isLoading} onClick={refresh}
            iconLeft="M1 4v6h6M23 20v-6h-6M20.49 9A9 9 0 005.64 5.64L1 10M23 14l-4.64 4.36A9 9 0 013.51 15">
            Actualizar
          </Button>
          <Button variant="primary" size="sm" onClick={() => navigate(ROUTES.NUEVA_RECEPCION)}
      ...
      {error && (
        <div className="rp-error">
          ...
          <button className="rp-error-retry" onClick={refresh}>Reintentar</button>
        </div>
      )}
      ...
      <div className="rp-filters">
        <div className="rp-search-wrap">
          <svg className="rp-search-icon" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#475569" strokeWidth="2" aria-hidden="true">
          ...
          {search && (
            <button className="rp-search-clear" onClick={() => setSearch("")} aria-label="Limpiar búsqueda">
        ...
        <select className="rp-select" data-empty={estadoFilter === ""}
          value={estadoFilter}
      ...
      <div className="rp-chips-row">
        <p className="rp-count">{filtered.length} recepcion{filtered.length !== 1 ? "es" : ""}</p>
        {hasFilter && (
          <button className="rp-clear-btn" onClick={() => { setSearch(""); setEstadoFilter(""); }}>
            Limpiar filtros
          </button>
        )}
        <div className="rp-chips">
          {Object.entries(ESTADO_CFG).map(([k, cfg]) => {
            const all   = (isMock ? MOCK_RECEPCIONES : recepciones);
            const count = all.filter(r => r.estado === Number(k)).length;
            if (!count) return null;
            const active = estadoFilter === Number(k);
            const dimmed = estadoFilter !== "" && !active;
            return (
              <button key={k} className="rp-chip" data-dimmed={dimmed}
                style={{ background: cfg.bg, color: cfg.color }}
                onClick={() => setEstadoFilter(active ? "" : Number(k) as EstadoRecepcion)}>
            ...
            <tr className="rp-thead-row">
              {[
                ...
                <th key={label} className={`rp-th ${cls}`}>
                  {label && <span className="rp-th-label">{label}</span>}
                </th>
              ))}
            </tr>
          </thead>
              ...
              ? Array.from({ length: 6 }).map((_, i) => (
                  <tr key={i}>
                    <td className="rp-skeleton-td" colSpan={7}>
                      <div className="rp-skeleton-line" style={{ height: "0.875rem", width: `${50 + (i % 3) * 15}%` }} />
                    </td>
                  </tr>
                ))
              : filtered.length === 0
                ? (
                  <tr>
                    <td className="rp-empty" colSpan={7}>
                      {hasFilter ? "Sin resultados para los filtros actuales." : "Sin recepciones registradas."}
                    </td>
                  </tr>
                )
                : filtered.map(rec => (
